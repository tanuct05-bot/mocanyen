...
async function fetchProducts() {
  const r = await fetch('https://mocanyen.vercel.app/data.json', { cache: 'no-store' });
...
module.exports = async (req, res) => {
  ...
  const orderId = 'MAY' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).slice(2, 5).toUpperCase();
  const order = {
    id: orderId,
